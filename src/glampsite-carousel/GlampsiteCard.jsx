...
export default function GlampsiteCard({ place }) {
  ...
  return (
    <div className="min-w-[220px] select-none max-w-[220px] w-[65vw] sm:w-[220px] self-stretch flex flex-col">
      <div className="w-full">
        <img
          src={place.thumbnail || 'https://picsum.photos/400/400'}
          alt={place.name}
      ...
      <div className="mt-3 flex flex-col flex-1 flex-auto">
        ...
        <div className="text-xs mt-1 text-black/60 flex items-center gap-1">
          ...
          {place.city ? (
            <>
              <MapPin className="h-3 w-3 ml-1" aria-hidden="true" />
        ...
        <div className="mt-5">
          <button
            type="button"
            className="cursor-pointer inline-flex items-center rounded-full bg-[#10B981] text-white px-4 py-1.5 text-sm font-medium hover:opacity-90 active:opacity-100"
          >
            View site
          </button>
        </div>
